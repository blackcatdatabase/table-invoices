...
<div style='font-size:13px;opacity:0.92;'>Lineage heat: 2 outbound / 1 inbound · ✅ No engine drift detected · Index coverage: ready · PII: 3 · Changelog: fresh (18 d)</div>
</div>
...
- Pkg folder: [packages\invoices](https://github.com/blackcatacademy/blackcat-database/blob/main/packages\invoices)
...
graph LR
  ...
  invoices -->|FK| orders["orders"]:::link
  invoices -->|FK| tenants["tenants"]:::accent
  invoice_items["invoice_items"]:::inbound -->|FK| invoices
...
- Inbound (relies on this): "invoice_items"
...
## Usage
- Install/upgrade schema: pwsh -NoLogo -NoProfile -File scripts/schema-tools/Migrate-DryRun.ps1 -Package invoices -Apply
