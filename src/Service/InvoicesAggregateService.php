...
namespace BlackCat\Database\Packages\Invoices\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\Invoices\Dto\InvoiceDto;
use BlackCat\Database\Packages\Invoices\Mapper\InvoiceDtoMapper;
 ...
 */
final class InvoicesAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private InvoiceRepository $invoiceRepo
    ) {
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\Invoices\ModuleException("Record {$idStr} not found for locking.");
            }
