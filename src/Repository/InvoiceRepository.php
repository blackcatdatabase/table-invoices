...
use BlackCat\Database\Packages\Invoices\Repository\InvoiceRepositoryInterface;
...
class InvoiceRepository implements InvoiceRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'tenant_id', 'invoice_number' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\Invoices\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'order_id', 'variable_symbol', 'issue_date', 'due_date', 'subtotal', 'discount_total', 'tax_total', 'total', 'currency', 'qr_data' ]);
          return count($rows);
        }
    ...
    public function getByTenantIdAndInvoiceNumber(int $tenantId, string $invoiceNumber, bool $asDto = false): array|\BlackCat\Database\Packages\Invoices\Dto\InvoiceDto|null {
        return $this->getByUnique([ 'tenant_id' => $tenantId, 'invoice_number' => $invoiceNumber ], $asDto);
    }
    ...
    public function getIdByTenantIdAndInvoiceNumber(int $tenantId, string $invoiceNumber) {
        $row = $this->getByTenantIdAndInvoiceNumber($tenantId, $invoiceNumber, false);
