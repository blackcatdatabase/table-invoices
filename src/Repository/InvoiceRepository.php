...
namespace BlackCat\Database\Packages\Invoices\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\Invoices\Definitions;
use BlackCat\Database\Packages\Invoices\Criteria;
use BlackCat\Database\Packages\Invoices\Dto\InvoiceDto as Dto;
use BlackCat\Database\Packages\Invoices\Mapper\InvoiceDtoMapper as RowMapper;
...
final class InvoiceRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\Invoices\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'tenant_id', 'invoice_number' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\Invoices\Definitions::table());
            return $bulk->upsertMany($rows, [ 'tenant_id', 'invoice_number' ], [ 'order_id', 'variable_symbol', 'issue_date', 'due_date', 'subtotal', 'discount_total', 'tax_total', 'total', 'currency', 'qr_data' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Invoices\Dto\InvoiceDto|null */
    public function getByTenantIdAndInvoiceNumber(int $tenantId, string $invoiceNumber, bool $asDto = false): array|\BlackCat\Database\Packages\Invoices\Dto\InvoiceDto|null {
        $row = $this->getByUnique([ 'tenant_id' => $tenantId, 'invoice_number' => $invoiceNumber ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\Invoices\Mapper\InvoiceDtoMapper::fromRow($row);
    }
    public function existsByTenantIdAndInvoiceNumber(int $tenantId, string $invoiceNumber): bool {
        $where = 't.' . Ident::q($this->db, 'tenant_id') . ' = :uniq_tenant_id' . ' AND ' . 't.' . Ident::q($this->db, 'invoice_number') . ' = :uniq_invoice_number';
        return $this->exists($where, [ 'uniq_tenant_id' => $tenantId, 'uniq_invoice_number' => $invoiceNumber ]);
    }
    /** @return int|string|null */
    public function getIdByTenantIdAndInvoiceNumber(int $tenantId, string $invoiceNumber) {
        $row = $this->getByTenantIdAndInvoiceNumber($tenantId, $invoiceNumber);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Invoices\Dto\InvoiceDto|null */
    public function getByTenantIdAndId(int $tenantId, int $id, bool $asDto = false): array|\BlackCat\Database\Packages\Invoices\Dto\InvoiceDto|null {
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Invoices\Dto\InvoiceDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\Invoices\Dto\InvoiceDto|null {
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Invoices\Dto\InvoiceDto|null */
    public function getByIdForTenant(int|string $id, int|string $tenantId, bool $asDto = false): array|\BlackCat\Database\Packages\Invoices\Dto\InvoiceDto|null {
